...
if ( ! class_exists( 'fmwp\admin\Version_Template_List_Table' ) ) {
	...
	class Version_Template_List_Table extends WP_List_Table {
		...
		/**
		 * FMWP_Versions_List_Table constructor.
		 *
		 * @param array $args
		 */
		...
		public function prepare_items() {
			...
			$templates = get_option( 'fmwp_template_statuses', array() );
		...
		public function column_template( $item ) {
			$output  = esc_html__( 'Core path - ', 'forumwp' );
			$output .= $item['core_file'] . '<br>';
			$output .= esc_html__( 'Theme path - ', 'forumwp' );
		...
		public function column_status( $item ) {
			$icon = 1 === $item['status_code'] ? 'fmwp-notification-is-active dashicons-yes' : 'dashicons-no-alt';
			return $item['status'] . ' <span class="dashicons fmwp-notification-status ' . esc_attr( $icon ) . '"></span>';
		}
	}
}
